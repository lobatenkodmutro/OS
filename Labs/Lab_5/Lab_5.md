...
During the laboratory work, practical experience with the Bash shell in the Linux environment was gained. Navigation of the file system using the pwd, cd, and ls commands was mastered, and the difference between absolute and relative paths was learned.

In addition, key operations for managing files and directories were practiced: creating, copying, moving, renaming, and deleting file system objects using the mkdir, touch, cp, mv, and rm commands. The use of glob characters for flexible file search by patterns was studied, as well as redirecting output with the echo command to write data directly to a file. The acquired skills form a solid foundation for confident work with Linux in command line mode without a graphical interface.
