...
Примітка: зображення в роботі взяті з додаткових інтернет-джерел
...
| № | Слово | Пояснення |
| :--- | :--- | :--- |
| 1 | **Command Line Interface (CLI)** | Текстове середовище для управління комп'ютером за допомогою спеціальних команд замість графічних вікон. |  
| 2 | **Kernel** | Основний системний компонент, що виступає центральним контролером всього, що відбувається на комп'ютері. |  
| 3 | **Process** | Активна програма під час її виконання, яка потребує виділення оперативної пам'яті. |  
| 4 | **Multitasking** | Здатність системи ділити час процесора між кількома завданнями для їх одночасного виконання. |  
| 5 | **Desktop Application** | Програма з візуальним інтерфейсом, створена для виконання завдань кінцевим користувачем. |  
| 6 | **Server Application** | Програмне забезпечення, що працює у фоні та обробляє мережеві запити від інших клієнтів. |  
| 7 | **Compiler** | Системна утиліта, яка перетворює зрозумілий програмісту код у машинні інструкції. |  
| 8 | **Resource Allocation** | Механізм розподілу ресурсів обладнання (пам'яті, дискового простору, часу ЦП) між програмами. |  
| 9 | **Terminal / X-term** | Програма-емулятор, що надає текстовий інтерфейс консолі всередині графічної оболонки користувача. |  
| 10 | **Source Code** | Набір текстових інструкцій програми, які комп'ютер використовує для роботи. |  
| 11 | **Application Programming Interface (API)** | Набір стандартних функцій та правил, через які сторонні програми взаємодіють з ОС. |  
| 12 | **Network File Share** | Технологія надання спільного доступу до файлів і папок через локальну мережу. |
